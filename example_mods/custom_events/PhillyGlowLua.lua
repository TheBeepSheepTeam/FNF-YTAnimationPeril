...
function onCreate()
	...
	if (inPhilly) then
		if (week7) then
			...
			for i = 0, getProperty("members.length") - 1 do
				...
				local isStreet =
					key:endsWith("street.png")
				
				local isCity =
					key:endsWith("city.png")
			...
			if (foundStreet) then
				makeLuaSprite("phillyStreet", "street", type(x) == "number" and x or -50, type(y) == "number" and y or 50)
	...
	if (inPhilly) then
		makeQSprite("phillyWindowEvent", "window", phillyWindowX, phillyWindowY, .3, .3)
